...
export async function GET(req: Request) {
  try {
    ...
    const [totalUsers, activeUsers, deviceStatsByDay, latestUsers] = await Promise.all([
      prisma.user.count(),
      prisma.token.count({
        where: {
    ...
    const deviceData = Array.from({ length: 7 }, (_, i) => {
      ...
      const dayStats = deviceStatsByDay.filter(
        (stat) => new Date(stat.createdAt).toISOString().split('T')[0] === dateStr,
      );
      ...
      return {
        ...
        other: dayStats.reduce((acc, { agent, _count }) => {
          const lowerAgent = agent?.toLowerCase() || '';
          return (
            acc + (!/windows|macintosh|linux|mobile|android|ios/.test(lowerAgent) ? _count : 0)
          );
        }, 0),
      };
